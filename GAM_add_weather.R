...
my.train = data.frame(train_with_weather[,list(WnvPresent, week, Tavg, Tmax, Tmin, Block, Species2, Latitude, Longitude)])
...
## GAM modelling
fitCv = gam(WnvPresent ~ week + Block + Tavg + Tmax + Tmin + Species2 + lo(Latitude, Longitude),
...
write.csv(submissionFile,"Output/GAM_with_weather.csv",row.names=FALSE,quote=FALSE)
