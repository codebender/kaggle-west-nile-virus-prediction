...
my.x = data.frame(x[,list(WnvPresent, dWeek, Block, Species2, Latitude, Longitude)])
x1<-my.x[x$dYear!=2011,]
x2<-my.x[x$dYear==2011,]
x2$Block[x2$Block==29] <- 28
x2$Block[x2$Block==34] <- 33

## GAM modelling
require(gam)
fitCv = gam(WnvPresent ~ s(dWeek) + Block + Species2 + lo(Latitude, Longitude),
...
write.csv(submissionFile,"Output/GAM_with_block.csv",row.names=FALSE,quote=FALSE)
